...
Docker Compose는 보통 한 번에 여러 개의 도커 컨테이너를 띄워야 할 때 사용하는데, 

이 외에도 YML파일 안에서 컨테이너의 여러 설정들을 추가로 관리할 수 있어서 편리합니다. 

저희는 컨테이너를 여러개 띄워서 관리하는 것은 아니지만,

위의 블루-그린-배포 에서 블루 컨테이너와 그린 컨테이너를 구분하기 위해서 사용합니다.

**앞의 그림과는 반대로 호스트의 8081포트와 연결된 것을 블루,**

**호스트의 8082포트와 연결된 것이 그린 으로 되어있으니 유의해주세요.**
...
별도의 태그를 안 붙이고, 기본 태그인 latest 태그로 덮어쓰는 방식으로 사용하면 이런 조잡한 짓을 할 필요가 없었을 텐데..... 
...
...
DATASOURCE_URL=실제_DATASOURCE_URL
DATASOURCE_USERNAME=실제_DATASOURCE_USERNAME
DATASOURCE_PASSWORD=실제_DATASOURCE_PASSWORD
OAUTH_GOOGLE_CLIENT_SECRET=실제_구글_OAUTH2_클라이언트_시크릿
...
